...
const make_logseqqer = () => {
  ...
  const handle = ({action, space, data}) => {
    ...
    super_handler
    ((_) => (_) => (_) => (a) => { console.error('action not found', a) })
    ( [ handler('logseq')(handle_logseq)
      , handler('app')(handle_app)
      , handler('editor')(handle_editor)
      , handler('db')(handle_db)
...
const logseq_select =
  ...
  (match_space) =>
  logg
  . filter(({action}) => deepEqual(match_action, action)) 
  . filter(({space}) => deepEqual(match_space, space))
...
const derun_logseq =
  (de_main) =>
  logseq.beforeunload(async () => de_main())
  // logseq.beforeunload((async () => de_main())().catch(console.error))
